...
//接收表單資料
  ...
  //依科目數設定接收 科目名稱 及 學分 資料廻圈 並 運用此廻圈計算 總學分 數
...
for($x=1;$x<=$nu;$x++)        //$x代表座號，$nu為人數資料
{
  ...
  for($a=1;$a<=($counu+1);$a++)     //$a代表: 1 姓名,2~$counu+1 各科目分數
  {
    ...
    $scor[$x][$a]=$_POST[$midata];    //接收對應表單名稱
    //計算總分(累加) 及 實得學分
    if($a<>1)
    {
       $scor[$x][$counu+2]+=$scor[$x][$a]*$cousc[$a-1];     //總分累加(各科分數*學分) 註:學分陣列變數索引值比分數索引值少1 
       if($scor[$x][$a]>=60)
       {
          $scor[$x][$counu+5]+=$cousc[$a-1];                //實得學分累加    註:學分陣列變數索引值比分數索引值少1  
       }
    }
