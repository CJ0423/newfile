<?php
  //// 設定資料名稱陣列  ///////////
  $daname=array("kind","prno","prna","price","sty","quan","saquan");
  $workitem=$_POST["send"];
  ...
  elseif($workitem=="庫存產品進貨")
  {
     ...
     $data[5]=$_POST[$daname[5]];
        ...
        ////  將購買數量加至 product 資料表的對應項目中 /////////////////       
           ...
           $sql = "UPDATE product SET 庫存量 = '$data[5]' WHERE 產品編號 = '$data[1]'";
...
/////// 接收上傳檔案並設定圖檔名稱/////////////////////
				...
				{
					//搬移檔案
					$upload_file = "graph\\" . $_FILES["fname"]["name"];
					move_uploaded_file($_FILES["fname"]["tmp_name"], $upload_file);
        ...
        ////  將資料新增至 hkco資料庫的 product 資料表中 /////////////////
           ...
           $sql="INSERT INTO product(kind1, 產品編號, 產品名稱, 圖檔, 單價, 規格, 庫存量, 安全存量) 
                      VALUES('$data[0]','$data[1]','$data[2]','$fname','$data[3]','$data[4]','$data[5]','$data[6]')";
