...
function acep($coti,$ank)  //定義接收猜測及比對結果資料函數
{
   if($coti>=2)            //如果猜測次超過1次，接收前面已猜測次數的資料
   {
    for($a=1;$a<$coti;$a++)
     {
      $alb="anb".$a;  
...
function decrypt($tu)       //定義題目資料解密函數
{
  ///////////  1.資料倒置
...
//接收資料
 ...
 $bn=acep($coti,$ank);       //接收前面已猜測次數的資料

//比較本次猜答結果
$tu=str_split($tp);           //拆解題目數字

$tu=decrypt($tu);             // 題目資料解密   
