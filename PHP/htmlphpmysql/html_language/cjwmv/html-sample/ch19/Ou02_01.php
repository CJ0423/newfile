<HTML>
	...
	<BODY>
		<?php
                 ...
                 //拆解 $de 為 $de[0](讀取資料用) 及 $de[1](列印報表用)---------增加
                   $de=explode(";",$desou);
...
mysqli_query($link,"SET NAMES UTF8");
                 ...
                 //讀取資料sql語法設定                ----------------------修改增加
                     if($de[0]=="０")
                      ...
                      {
			$sql = "SELECT * FROM employee where 部門代號 like '$fi'";
                      }

		$result = mysqli_query($link, $sql);

	         //擷取指定部門職員工基本資料(部門代號、姓名、職稱、月薪、年假天數)
                        $k=0;  //記錄資料筆數
...
array_multisort($paper["dp"],SORT_STRING,SORT_ASC,$paper["mp"],SORT_NUMERIC,SORT_DESC,$paper["na"],SORT_NUMERIC,SORT_ASC,$paper["jo"],$paper["d1"]);
			...
			while ($row1 = mysqli_fetch_array($result1, MYSQL_NUM))
			{
                 ...
                 //列印 成果報表 @$paper[][]資料
                 //  標題列 -----------------------------------------------------------修改 
 	                  echo "<CENTER><h3><u>頂新資訊公司民國八十九年".$de[1]."員工未休假奬金統計報表</u></h3>";
                 //  報表
                          echo "<table >";
                 //  報表 日期列
                          ...
                          echo "<tr>";
                            echo "<td colspan='7'>";
                              echo "<table border='3' frame='hsides' rules='none'><tr>";
                                 ...
                                 echo "<td width='80' align='right'>年假天數</td>";
                                 echo "<td width='80' align='right'>未休天數</td>";
                                 echo "<td width='100' align='right'>未休假奬金</td>";
                                 echo "<td width='50' align='center'>備註</td>";
                              echo "</tr></table>";
                            echo "</td>";
                          echo "</tr>";
                  //  資料設定($dep:部門代號,$mdep:部門加總,$tdep:總加總)    99列
                        $dep="";
                        $mdep=0;
                        $tdep=0;
                  //  各課資料
                      ...
                      // 列印部門加總
                         ...
                         $mdep1=po3($mdep);
                                       echo "<td colspan='4' align='right' width='330'>".$mdep1."</td>";
                                       echo "<td width='50'></td>";
                                   echo "</tr></table>";
                               echo "</td></tr>";
                               $mdep=0;                //部門加總設定 
                      ...
                      //設定課別標題資料----------------------------增加
  $depna=array("A01"=>"董事長室","B01"=>"總經理室","C01"=>"研發一課","C02"=>"研發二課","C03"=>"研發三課","D01"=>"業務一課","D02"=>"業務二課","D03"=>"業務三課","D04"=>"業務四課","E01"=>"採購部","F01"=>"維修部","G01"=>"資訊部","H01"=>"企劃部","I01"=>"人事部","J01"=>"行政部","K01"=>"會計部","L01"=>"圖書室");
  
                       // 列印課別標題------------------------------增加修改
                     echo "<tr><td colspan='7'>$depna[$dep]</td></tr>";
                     ...
                     // 列印課別資料
                        ...
                        //echo "<td width='20'>".@$paper[dp][$i]."</td>";
                              ...
                              echo "<td width='80' align='right'>".@$paper[d1][$i]."</td>";
                              echo "<td width='80' align='right'>".@$paper[d2][$i]."</td>";
                      ...
                      // 列印總金額(兩空白列)
                         ...
                         $tdep1=po3($tdep);
                                       echo "<td colspan='4' align='right' width='330'>".$tdep1."</td>";
                                       echo "<td width='50'></td>";
                                   echo "</tr></table>";
                               echo "</td></tr>";
